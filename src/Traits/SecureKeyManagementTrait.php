...
trait SecureKeyManagementTrait {
  ...
  protected function getSecureKey($key_name, $direct_key = '', $type = 'Key', $required = FALSE) {
    ...
    if (!empty($key_name) && $key_repository) {
        ...
        if (!empty($key_value)) {
          ...
          if (!empty($direct_key)) {
            if (isset($this->logger)) {
              $this->logger->warning('@type key "@key" is empty. Using direct value as fallback.', [
                '@type' => $type,
                '@key' => $key_name,
              ]);
            }
      ...
      if ($required) {
        ...
        if (isset($this->logger)) {
          $this->logger->warning($message . ' Falling back to direct value.');
        }
        return $direct_key;
      }
    }
  ...
  /**
   * Adds password key fields to the connection form if Key module is available.
   *
   * @param array &$form
   ...
   */
  protected function validateApiKeyConfig(array $values, $key_name_field, $direct_key_field, $label = 'API key') {
  ...
  protected function getDatabasePassword() {
    ...
    return $this->getSecureKey($password_key, $direct_password, 'Database password');
  }

  /**
   * Gets Azure API key considering key configuration.
   *
   * @return string
   *   The Azure API key.
   */
  protected function getAzureApiKey() {
    ...
    return $this->getSecureKey($api_key_name, $direct_key, 'Azure API key');
  }
