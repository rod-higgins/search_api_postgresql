...
/**
 * Tests for EmbeddingAnalyticsService.
 *
 * @group search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Service\EmbeddingAnalyticsService
 */
class EmbeddingAnalyticsServiceTest extends TestCase {
  /**
   * The EmbeddingAnalyticsService instance under test.
   */
  protected ${lcfirst(EmbeddingAnalyticsService)};
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/Service/EmbeddingAnalyticsService.php';
    ...
    try {
      ...
      $this->{lcfirst(EmbeddingAnalyticsService)} = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  protected function createInstance() {
    ...
    return new EmbeddingAnalyticsService(
          $this->logger
      );
  }
  ...
  public function testClassInstantiation() {
    if (!$this->{lcfirst(EmbeddingAnalyticsService)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\Service\EmbeddingAnalyticsService',
          $this->{lcfirst(EmbeddingAnalyticsService)}
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(EmbeddingAnalyticsService)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->{lcfirst(EmbeddingAnalyticsService)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(EmbeddingAnalyticsService)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(EmbeddingAnalyticsService)}->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)}, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)});
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(EmbeddingAnalyticsService)});
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)}, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)});
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(EmbeddingAnalyticsService)}, 'test_data');
