...
use Drupal\search_api_postgresql\Service\DegradationMessageService;
use Psr\Log\LoggerInterface;
use PHPUnit\Framework\TestCase;

/**
 * Tests for DegradationMessageService.
 *
 * @group search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Service\DegradationMessageService
 */
class DegradationMessageServiceTest extends TestCase {
  /**
   * The DegradationMessageService instance under test.
   */
  protected ${lcfirst(DegradationMessageService)};
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/Service/DegradationMessageService.php';
    ...
    try {
      ...
      $this->{lcfirst(DegradationMessageService)} = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  protected function createInstance() {
    ...
    return new DegradationMessageService(
          $this->logger
      );
  }
  ...
  public function testClassInstantiation() {
    if (!$this->{lcfirst(DegradationMessageService)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\Service\DegradationMessageService',
          $this->{lcfirst(DegradationMessageService)}
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(DegradationMessageService)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->{lcfirst(DegradationMessageService)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(DegradationMessageService)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(DegradationMessageService)}->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(DegradationMessageService)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(DegradationMessageService)}, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(DegradationMessageService)});
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(DegradationMessageService)});
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(DegradationMessageService)}, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(DegradationMessageService)});
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(DegradationMessageService)}, 'test_data');
