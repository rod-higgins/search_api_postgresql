...
use Drupal\search_api_postgresql\Service\AzureCognitiveServicesEmbeddingService;
use Psr\Log\LoggerInterface;
use PHPUnit\Framework\TestCase;

/**
 * Tests for AzureCognitiveServicesEmbeddingService.
 *
 * @group search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Service\AzureCognitiveServicesEmbeddingService
 */
class AzureCognitiveServicesEmbeddingServiceTest extends TestCase {
  /**
   * The AzureCognitiveServicesEmbeddingService instance under test.
   */
  protected ${lcfirst(AzureCognitiveServicesEmbeddingService)};
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/Service/AzureCognitiveServicesEmbeddingService.php';
    ...
    try {
      ...
      $this->{lcfirst(AzureCognitiveServicesEmbeddingService)} = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  protected function createInstance() {
    ...
    return new AzureCognitiveServicesEmbeddingService(
          $this->logger
      );
  }
  ...
  public function testClassInstantiation() {
    if (!$this->{lcfirst(AzureCognitiveServicesEmbeddingService)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\Service\AzureCognitiveServicesEmbeddingService',
          $this->{lcfirst(AzureCognitiveServicesEmbeddingService)}
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(AzureCognitiveServicesEmbeddingService)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->{lcfirst(AzureCognitiveServicesEmbeddingService)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(AzureCognitiveServicesEmbeddingService)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(AzureCognitiveServicesEmbeddingService)}->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)}, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)});
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(AzureCognitiveServicesEmbeddingService)});
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)}, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)});
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(AzureCognitiveServicesEmbeddingService)}, 'test_data');
