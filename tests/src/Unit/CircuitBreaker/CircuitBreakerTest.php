...
use Drupal\search_api_postgresql\CircuitBreaker\CircuitBreaker;
 ...
 * @covers \Drupal\search_api_postgresql\CircuitBreaker\CircuitBreaker
 */
class CircuitBreakerTest extends TestCase {
  /**
   * The CircuitBreaker instance under test.
   */
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/CircuitBreaker/CircuitBreaker.php';
    ...
    try {
      ...
      $this->circuitBreaker = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  public function testClassInstantiation() {
    ...
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\CircuitBreaker\CircuitBreaker',
          $this->circuitBreaker
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->circuitBreaker, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->circuitBreaker->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->circuitBreaker->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->circuitBreaker);
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->circuitBreaker, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->circuitBreaker);
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->circuitBreaker);
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->circuitBreaker, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->circuitBreaker);
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->circuitBreaker, 'test_data');
