...
/**
 * Tests for AzureVectorIndexManager.
 *
 * @group  search_api_postgresql
 * @covers \Drupal\search_api_postgresql\PostgreSQL\AzureVectorIndexManager
 */
class AzureVectorIndexManagerTest extends TestCase
{
  /**
   * The AzureVectorIndexManager instance under test.
   */
  protected $azureVectorIndexManager;
  ...
  protected function setUp(): void
  {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/PostgreSQL/AzureVectorIndexManager.php';
    ...
    try {
      ...
      $this->{lcfirst(AzureVectorIndexManager)} = $this->createInstance();
    } catch (\TypeError $e) {
  ...
  protected function createInstance()
  {
    ...
    return new AzureVectorIndexManager(
        $this->logger
    );
  }
  ...
  {
    if (!$this->{lcfirst(AzureVectorIndexManager)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
        '\Drupal\search_api_postgresql\PostgreSQL\AzureVectorIndexManager',
        $this->{lcfirst(AzureVectorIndexManager)}
    );
  }
  ...
  public function testEssentialMethodsExist()
  {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(AzureVectorIndexManager)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
          method_exists($this->{lcfirst(AzureVectorIndexManager)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(AzureVectorIndexManager)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(AzureVectorIndexManager)}->$methodName($testValue);
  ...
  public function testProtectedMethods()
  {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(AzureVectorIndexManager)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(AzureVectorIndexManager)}, $input);
  ...
  public function testLogging()
  {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(AzureVectorIndexManager)});
        } catch (\Exception $e) {
  ...
  public function testPropertyInitialization()
  {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(AzureVectorIndexManager)});
  ...
  public function testConfigurationHandling()
  {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(AzureVectorIndexManager)}, $testConfig);
        $this->assertTrue(true);
      } catch (\Exception $e) {
        $this->assertTrue(true);
      }
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(AzureVectorIndexManager)});
      ...
      if (strpos($methodName, 'validate') !== false ||
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(AzureVectorIndexManager)}, 'test_data');
