 ...
 * @covers \Drupal\search_api_postgresql\PostgreSQL\VectorQueryBuilder
 */
class VectorQueryBuilderTest extends TestCase
{
  /**
   * The VectorQueryBuilder instance under test.
   */
  protected $vectorQueryBuilder;
  ...
  protected function setUp(): void
  {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/PostgreSQL/VectorQueryBuilder.php';
    ...
    try {
      ...
      $this->{lcfirst(VectorQueryBuilder)} = $this->createInstance();
    } catch (\TypeError $e) {
  ...
  {
    if (!$this->{lcfirst(VectorQueryBuilder)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
        '\Drupal\search_api_postgresql\PostgreSQL\VectorQueryBuilder',
        $this->{lcfirst(VectorQueryBuilder)}
    );
  }
  ...
  public function testEssentialMethodsExist()
  {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(VectorQueryBuilder)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
          method_exists($this->{lcfirst(VectorQueryBuilder)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(VectorQueryBuilder)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(VectorQueryBuilder)}->$methodName($testValue);
  ...
  public function testProtectedMethods()
  {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(VectorQueryBuilder)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(VectorQueryBuilder)}, $input);
  ...
  public function testLogging()
  {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(VectorQueryBuilder)});
        } catch (\Exception $e) {
  ...
  public function testPropertyInitialization()
  {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(VectorQueryBuilder)});
  ...
  public function testConfigurationHandling()
  {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(VectorQueryBuilder)}, $testConfig);
        $this->assertTrue(true);
      } catch (\Exception $e) {
        $this->assertTrue(true);
      }
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(VectorQueryBuilder)});
      ...
      if (strpos($methodName, 'validate') !== false ||
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(VectorQueryBuilder)}, 'test_data');
