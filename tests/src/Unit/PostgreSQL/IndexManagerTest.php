 ...
 * @covers \Drupal\search_api_postgresql\PostgreSQL\IndexManager
 */
class IndexManagerTest extends TestCase {
  /**
   * The IndexManager instance under test.
   */
  protected ${lcfirst(IndexManager)};
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/PostgreSQL/IndexManager.php';
    ...
    try {
      ...
      $this->{lcfirst(IndexManager)} = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  public function testClassInstantiation() {
    if (!$this->{lcfirst(IndexManager)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\PostgreSQL\IndexManager',
          $this->{lcfirst(IndexManager)}
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(IndexManager)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->{lcfirst(IndexManager)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(IndexManager)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(IndexManager)}->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(IndexManager)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(IndexManager)}, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(IndexManager)});
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(IndexManager)});
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(IndexManager)}, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(IndexManager)});
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(IndexManager)}, 'test_data');
