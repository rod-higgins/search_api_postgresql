...
/**
 * Tests for GracefulDegradationException.
 * {@inheritdoc}
 *
 * @group  search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Exception\GracefulDegradationException
 */
class GracefulDegradationExceptionTest extends TestCase
{
  /**
   * The GracefulDegradationException instance under test.
   */
  protected $gracefulDegradationException;
  ...
  protected function setUp(): void
  {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/Exception/GracefulDegradationException.php';
    ...
    try {
      ...
      $this->gracefulDegradationException = $this->createInstance();
    } catch (\TypeError $e) {
  ...
  protected function createInstance()
  {
    ...
    return new GracefulDegradationException(
        $this->logger
    );
  }
  ...
  {
    if (!$this->gracefulDegradationException) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
        '\Drupal\search_api_postgresql\Exception\GracefulDegradationException',
        $this->gracefulDegradationException
    );
  }
  ...
  public function testEssentialMethodsExist()
  {
    ...
    $reflection = new \ReflectionClass($this->gracefulDegradationException);
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
          method_exists($this->gracefulDegradationException, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->gracefulDegradationException->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->gracefulDegradationException->$methodName($testValue);
  ...
  public function testProtectedMethods()
  {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->gracefulDegradationException);
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->gracefulDegradationException, $input);
  ...
  public function testLogging()
  {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->gracefulDegradationException);
        } catch (\Exception $e) {
  ...
  public function testPropertyInitialization()
  {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->gracefulDegradationException);
  ...
  public function testConfigurationHandling()
  {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->gracefulDegradationException, $testConfig);
        $this->assertTrue(true);
      } catch (\Exception $e) {
        $this->assertTrue(true);
      }
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->gracefulDegradationException);
      ...
      if (strpos($methodName, 'validate') !== false ||
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->gracefulDegradationException, 'test_data');
