...
use Drupal\search_api_postgresql\Exception\SecurityExceptions;
use Psr\Log\LoggerInterface;
use PHPUnit\Framework\TestCase;

/**
 * Tests for SecurityExceptions.
 * {@inheritdoc}
 *
 * @group  search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Exception\SecurityExceptions
 */
class SecurityExceptionsTest extends TestCase
{
  /**
   * The SecurityExceptions instance under test.
   */
  ...
  protected function setUp(): void
  {
    ...
    try {
      ...
      $this->securityExceptions = $this->createInstance();
    } catch (\TypeError $e) {
  ...
  {
    if (!$this->securityExceptions) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
        '\Drupal\search_api_postgresql\Exception\SecurityExceptions',
  ...
  public function testEssentialMethodsExist()
  {
    ...
    $reflection = new \ReflectionClass($this->securityExceptions);
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
          method_exists($this->securityExceptions, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->securityExceptions->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->securityExceptions->$methodName($testValue);
  ...
  public function testProtectedMethods()
  {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->securityExceptions);
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->securityExceptions, $input);
  ...
  public function testLogging()
  {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->securityExceptions);
        } catch (\Exception $e) {
  ...
  public function testPropertyInitialization()
  {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->securityExceptions);
  ...
  public function testConfigurationHandling()
  {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->securityExceptions, $testConfig);
        $this->assertTrue(true);
      } catch (\Exception $e) {
        $this->assertTrue(true);
      }
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->securityExceptions);
      ...
      if (strpos($methodName, 'validate') !== false ||
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->securityExceptions, 'test_data');
