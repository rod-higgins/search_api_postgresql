...
/**
 * Tests for ComprehensiveExceptionFactory.
 *
 * @group search_api_postgresql
 * @covers \Drupal\search_api_postgresql\Exception\ComprehensiveExceptionFactory
 */
class ComprehensiveExceptionFactoryTest extends TestCase {
  /**
   * The ComprehensiveExceptionFactory instance under test.
   */
  protected ${lcfirst(ComprehensiveExceptionFactory)};
  ...
  protected function setUp(): void {
    ...
    // Load actual class.
    require_once __DIR__ . '/../../../../../../src/Exception/ComprehensiveExceptionFactory.php';
    ...
    try {
      ...
      $this->{lcfirst(ComprehensiveExceptionFactory)} = $this->createInstance();
    }
    catch (\TypeError $e) {
  ...
  protected function createInstance() {
    ...
    return new ComprehensiveExceptionFactory(
          $this->logger
      );
  }
  ...
  public function testClassInstantiation() {
    if (!$this->{lcfirst(ComprehensiveExceptionFactory)}) {
      $this->markTestSkipped('Class not instantiated');
    }
    $this->assertInstanceOf(
          '\Drupal\search_api_postgresql\Exception\ComprehensiveExceptionFactory',
          $this->{lcfirst(ComprehensiveExceptionFactory)}
      );
  }
  ...
  public function testEssentialMethodsExist() {
    ...
    $reflection = new \ReflectionClass($this->{lcfirst(ComprehensiveExceptionFactory)});
    ...
    foreach ($methods as $method) {
      $this->assertTrue(
            method_exists($this->{lcfirst(ComprehensiveExceptionFactory)}, $method->getName()),
    ...
    foreach ($methods as $method) {
      ...
      if (strpos($methodName, 'get') === 0) {
        if ($method->getNumberOfRequiredParameters() === 0) {
          try {
            $result = $this->{lcfirst(ComprehensiveExceptionFactory)}->$methodName();
      ...
      if (strpos($methodName, 'set') === 0) {
        if ($method->getNumberOfRequiredParameters() === 1) {
          try {
            ...
            $result = $this->{lcfirst(ComprehensiveExceptionFactory)}->$methodName($testValue);
  ...
  public function testProtectedMethods() {
    ...
    foreach ($methods as $method) {
      ...
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $result = $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)});
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 1) {
        foreach ($invalidInputs as $input) {
          try {
            $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)}, $input);
  ...
  public function testLogging() {
    ...
    foreach ($methods as $method) {
      if ($method->getNumberOfRequiredParameters() === 0) {
        try {
          $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)});
        }
        catch (\Exception $e) {
  ...
  public function testPropertyInitialization() {
    ...
    foreach ($properties as $property) {
      ...
      try {
        $value = $property->getValue($this->{lcfirst(ComprehensiveExceptionFactory)});
  ...
  public function testConfigurationHandling() {
    ...
    if ($reflection->hasMethod('setConfiguration')) {
      ...
      try {
        $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)}, $testConfig);
        $this->assertTrue(TRUE);
      }
      catch (\Exception $e) {
        $this->assertTrue(TRUE);
    ...
    if ($reflection->hasMethod('getConfiguration')) {
      ...
      try {
        $config = $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)});
      ...
      if (
        ...
        if ($method->getNumberOfRequiredParameters() <= 1) {
          try {
            ...
            $result = $method->invoke($this->{lcfirst(ComprehensiveExceptionFactory)}, 'test_data');
